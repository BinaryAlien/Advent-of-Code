...
let rec exclude index = function
  | [] -> failwith "exclude"
  | _ :: tail when index = 0 -> tail
  | head :: tail -> head :: exclude (index - 1) tail

let rec any = function [] -> false | true :: _ -> true | _ :: tail -> any tail

let safe report =
  let safe_without index _ = safe (exclude index report) in
  any (safe report :: List.mapi safe_without report)
